...
func (f PluginInstallFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PluginInstallMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PluginInstallMutation", m)
}

// The PluginMigrationHistoryFunc type is an adapter to allow the use of ordinary
// function as PluginMigrationHistory mutator.
type PluginMigrationHistoryFunc func(context.Context, *ent.PluginMigrationHistoryMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PluginMigrationHistoryFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PluginMigrationHistoryMutation); ok {
		return f(ctx, mv)
	}

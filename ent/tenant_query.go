...
// QueryPluginInstalls chains the current query on the "plugin_installs" edge.
func (tq *TenantQuery) QueryPluginInstalls() *PluginInstallQuery {
	query := (&PluginInstallClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithPluginInstalls tells the query-builder to eager-load the nodes that are connected to
// the "plugin_installs" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TenantQuery) WithPluginInstalls(opts ...func(*PluginInstallQuery)) *TenantQuery {
	query := (&PluginInstallClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := tq.withPluginInstalls; query != nil {
...
func (tq *TenantQuery) loadPluginInstalls(ctx context.Context, query *PluginInstallQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *PluginInstall)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(plugininstall.FieldTenantID)
	}
